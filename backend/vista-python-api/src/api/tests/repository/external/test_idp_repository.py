...
def test_get_user_by_id_returns_user_successfully(mock_boto_client, repository):
...
def test_get_user_by_id_does_not_exist_returns_none(mock_boto_client, repository):
    """Check path where user does not exist is handled correctly."""
    # mock existence of one user in admin group
    mock_boto_client.list_users.return_value = {"Users": []}
    user_id = str(uuid4())
...
def test_resend_user_invite_calls_cognito_and_email_repository(
    repository, mock_boto_client, mock_email_repository
):
