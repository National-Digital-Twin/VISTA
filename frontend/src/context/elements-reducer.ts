...
export interface State {
    ...
    /** Icon styles(?) */
    iconStyles: {};
    /** Max asset total CXNs(?) */
    maxAssetTotalCxns: 0;
    /** Asset criticality colour scale(?) */
    assetCriticalityColorScale: {};
    /** CXN criticality colour scale(?) */
...
export type Action =
    ...
    | {
          type: typeof ADD_DEPENDENCIES;
          dependencies: Dependency[];
      }
    | {
          type: typeof REMOVE_ELEMENTS_BY_TYPE;
          typeUri: string;
      }
    | {
          type: typeof FILTER_SELECTED_ELEMENTS | typeof RESET | typeof CLEAR_SELECTED;
      }
    | {
          type: typeof SELECT_ELEMENT | typeof MULTI_SELECT_ELEMENTS | typeof AREA_SELECTION;
          selectedElements: any[];
      }
    | {
          type: typeof UPDATE_ERRORS | typeof DISMISS_ERROR;
          error: string;
      };
...
const getAllCounts = (assets: Asset[]) => assets.map((asset) => asset.dependent.count);
const getAllCriticalitySums = (assets: Asset[]) => assets.map((asset) => asset.dependent.criticalitySum);

const elementsReducer = (state: State, action: Action) => {
    switch (action.type) {
        case ADD_ASSETS: {
        ...
        case MULTI_SELECT_ELEMENTS: {
            const selectedElements = getUniqueElements([...state.selectedElements, ...action.selectedElements]);
            return {
                ...state,
                selectedElements,
            };
        ...
        case UPDATE_ERRORS:
            return {
                ...state,
                errors: [...new Set([...state.errors, action.error])],
            };
        case DISMISS_ERROR:
            return {
                ...state,
