...
export type Action =
  ...
  | {
      type:
        | typeof FILTER_SELECTED_ELEMENTS
        | typeof RESET
        | typeof CLEAR_SELECTED;
    }
  | {
      type:
        | typeof SELECT_ELEMENT
        | typeof MULTI_SELECT_ELEMENTS
        | typeof AREA_SELECTION;
...
const getAllCounts = (assets: Asset[]) =>
  assets.map((asset) => asset.dependent.count);
const getAllCriticalitySums = (assets: Asset[]) =>
  assets.map((asset) => asset.dependent.criticalitySum);
  ...
  switch (action.type) {
    ...
    case REMOVE_ELEMENTS_BY_TYPE: {
      ...
      const dependencies = state.dependencies.filter(
        (dependency) =>
          dependency.dependent.type !== typeUri ||
          dependency.provider.type !== typeUri,
      );
        ...
        (selectedElement) => {
          return isAsset(selectedElement)
            ...
            : state.dependencies.some(
                (dependency) => dependency.uri === selectedElement.uri,
              );
        },
      );
      return {
