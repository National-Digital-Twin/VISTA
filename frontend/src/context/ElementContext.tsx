...
export const ElementsContext = React.createContext<ElementsContextItems | null>(
  null,
);
...
export function ElementsProvider({ children }: ElementsProviderProps) {
  ...
  const drawnFloodAreaFeatures = useSharedStore(
    (state) => state.floodAreaFeatures,
  );
  const selectedDrawnFloodAreaFeatureIds = useSharedStore(
    (state) => state.selectedFloodAreaFeatureIds,
  );
  ...
  const liveFloodAreas = useMemo(
    () => (showLiveFloods ? liveFloodAreasRaw : []),
  ...
  const primaryAssetsAtRiskFromDrawn = useMemo(() => {
    ...
    const selectedDrawnFeatures = drawnFloodAreaFeatures.filter(
      (feature) => selectedDrawnFloodAreaFeatureIds[feature.id],
    );
  ...
  const primaryAssetsAtRiskFromReal = useMemo(() => {
    ...
    const cacheMisses = Object.fromEntries(
      features
        ...
        .map((feature) => [
          feature.properties.FWS_TACODE,
          findAssetsInPolygons({ polygons: [feature] }),
        ]),
    );
  ...
  const primaryAssetsAtRisk = useMemo(
    ...
    [
      primaryAssetsAtRiskFromDrawn,
      primaryAssetsAtRiskFromReal,
      primaryAssetsAtRiskFromLive,
    ],
  );
  ...
  const [dependenciesByFloodArea, assetsByFloodArea] = useMemo(() => {
    ...
    if (showSecondary && !isAssetsLoading) {
      ...
      } = getDependentAssets(primaryAssetsAtRisk);
  ...
  const assetCriticalities = useMemo(
    () =>
      Array.from(
        new Set(
          assetsByFloodArea.map((asset) => asset.dependent.criticalitySum),
        ),
  ...
  const [assetsMatchingSelectedTypes, dependenciesMatchingSelectedTypes] =
    useMemo(() => {
      ...
      const selectedTypes = Object.keys(selectedAssetTypes).filter(
        (assetType) => selectedAssetTypes[assetType],
      );
...

  {
    ...
    const dependencies = useMemo(
      () =>
        Array.from(
          new Set([
            ...dependenciesMatchingSelectedTypes,
            ...dependenciesByFloodArea,
          ]),
        ),
    ...
    const assets = useMemo(
      () =>
        getUniqueElements([
          ...
          ...assetsByFloodArea.filter(
            (asset) => asset.dependent.criticalitySum >= minCriticality,
          ),
        ]),
    ...
    const missingAssets = useMemo(
      () => assets.filter((asset: Asset) => !state.assets.includes(asset)),
      [assets, state.assets],
    );
    const missingDependencies = useMemo(
      () =>
        dependencies.filter(
          (dependency: Dependency) => !state.dependencies.includes(dependency),
        ),
      [dependencies, state.dependencies],
    );
    const additionalAssets = useMemo(
      () => state.assets.filter((asset: Asset) => !assets.includes(asset)),
      [assets, state.assets],
    );
    const additionalDependencies = useMemo(
      () =>
        state.dependencies.filter(
          (dependency: Dependency) => !dependencies.includes(dependency),
        ),
      [dependencies, state.dependencies],
    );
  ...
  const onElementClick = useCallback(
    (multiSelect: boolean, selectedElements: any[]) => {
      if (multiSelect) {
