...
import { useShowPointerCoords, useTogglePointerCoords } from '@/context/ShowPointerCoords';
...
export default function MapSettingsButton() {
    ...
    const { value: showSettingsMenu, toggle: toggleSettingsMenu, setFalse: hideSettingsMenu } = useBoolean(false);
    ...
    const { showCpsIconsForAssetTypes, toggleShowCpsIconsForAssetTypes } = useSharedStore();
    ...
    return (
        <Box ref={ref} sx={{ display: 'flex', justifyContent: 'end', pointerEvents: 'auto' }}>
            ...
            <ToolbarButton title="Settings menu" onClick={toggleSettingsMenu} svgSrc="icons/Settings.svg" />
        </Box>
    );
}

function ToggleSwitchControl({ name, onItemClick, checked }: Readonly<ToggleSwitchControlMenuItem>) {
    return (
