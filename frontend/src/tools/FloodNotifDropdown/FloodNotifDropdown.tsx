...
export default function SideButtons() {
    const { value: isOpen, setFalse: closeWidget, toggle: toggleWidget } = useBoolean(false);
    ...
    return (
        <Box sx={{ display: 'flex', justifyContent: 'end', pointerEvents: 'auto' }} ref={widgetRef}>
            {isOpen && (
                <Box boxShadow={4} className={styles.floodPanel}>
                    <FloodRiskAreas atRiskAreas={atRiskAreas} />
                    <Link component="button" variant="body1" onClick={closeWidget} className={styles.closeButton}>
                        Close
                    </Link>
                </Box>
            )}
