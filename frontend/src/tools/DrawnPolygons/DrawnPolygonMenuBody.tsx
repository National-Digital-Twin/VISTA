...
import { Box, IconButton, ListItemText, SvgIcon, TextField } from '@mui/material';
...
export default function DrawnPolygonMenuBody({ searchQuery = '', updateSelectedCount }: DrawnPolygonMenuBodyProps) {
    const { isError: isErrorFloodAreas, data: floodAreaNodes } = useFloodWatchAreas();

    const { setFeatures, toggleFeature, features, selectedFeatureIds, selected, ...drawingModeCallbacks } = useFloodAreaSharedStore();
    ...
    const handleDeleteFeature = (featureId: NonNullable<Feature['id']>) => {
        if (globalThis.confirm('Delete this drawn polygon?')) {
                    ...
                    >
                        <Box sx={{ display: 'flex', alignItems: 'left', flexGrow: 1 }}>
                            {editingPolygonId === feature.id ? (
                                ...
                                <>
                                    <Box>
                                        <ListItemText primary={feature.properties?.name ?? ` area ${index + 1}`} />
                                    </Box>
                                    <IconButton
                                        size="small"
                                        onClick={(event) => {
