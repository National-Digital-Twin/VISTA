...
export default function DynamicProximityMapElements() {
    ...
    const buildingLayerPaint = {
        'fill-extrusion-color': [
            'case',
            ['any', ...features.map(({ properties }) => ['<=', ['distance', point(properties.center)], properties.radiusInKm * 1000])],
            'blue',
            'green',
        ],
    ...
    return (
        <>
            <Layer id={buildingLayerId} source="esri" source-layer="TopographicArea_2" type="fill-extrusion" paint={buildingLayerPaint} />
            <Layer id={pointLayerId} source="esri" source-layer="TopographicPoint" type="symbol" paint={pointLayerPaint} />
        </>
    );
}
