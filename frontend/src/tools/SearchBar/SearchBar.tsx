...
export default function SearchBar() {
  ...
  const selectedFloodAreas = useSharedStore(
    (state: any) => state.selectedFloodAreas,
  );
  const selectedAssetTypes = useSharedStore(
    (state: any) => state.selectedAssetTypes,
  );
  ...
  const handleClickOutside = useCallback((event) => {
    if (
      ...
      !sideDropdownRef.current.contains(event.target) &&
  ...
  const hasSelectedAssetTypes = Object.values(selectedAssetTypes).some(
    (value) => value === true,
  );
