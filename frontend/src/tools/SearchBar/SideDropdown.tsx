...
export default forwardRef<HTMLDivElement, SideDropdownProps>(function SideDropdown({ searchQuery, className }: SideDropdownProps, ref) {
    const {
    ...
    return (
        <div className={classNames(styles.sideDropdown, className)} ref={ref}>
            {assessmentsData.map((assessment) => (
                <AssetTypes key={assessment.uri} assessment={assessment.uri} searchQuery={searchQuery} />
            ))}
        </div>
    );
});
