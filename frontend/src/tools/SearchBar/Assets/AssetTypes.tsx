...
import { faChevronUp, faChevronDown, faEye } from '@fortawesome/free-solid-svg-icons';
...
export default function AssetTypes({ assessment, searchQuery }: AssetTypesProps) {
...
function AssetTypeCategory({ category, selectedCategory, assets, onCategoryClick }: AssetTypeCategoryProps) {
    const selectedAssetTypes = useStore((state) => state.selectedAssetTypes);

    const isCategoryActive = assets.some((asset) => selectedAssetTypes[asset.type]);
