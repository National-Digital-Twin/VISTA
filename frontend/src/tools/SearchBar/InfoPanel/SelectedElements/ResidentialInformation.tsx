  ...
  return (
    <div className={classNames("flex flex-col gap-y-1", className)}>
      <div className="flex items-center justify-between text-whiteSmoke-300">
        ...
        {totalResidences > LIMIT && (
          <p className="text-sm">{totalResidences} addresses found</p>
        )}
      </div>
      <Addresses
...
function Addresses({
  ...
  const WRAPPER_CLASSNAMES =
    "flex flex-col gap-y-2 bg-black-100 p-2 rounded-sm";
  ...
  if (isEmpty(residences)) {
    return (
      <p className={WRAPPER_CLASSNAMES}>Residential information not found</p>
    );
  }
