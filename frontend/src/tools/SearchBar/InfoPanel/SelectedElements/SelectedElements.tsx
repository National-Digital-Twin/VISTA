...
export default function SelectedElements({ selectedElements }: SelectedElementsProps) {
    ...
    if (totalSelected > 0) {
        return <ElementsList selectedElements={selectedElements} totalSelected={totalSelected} />;
    }
...
function ElementsList({ selectedElements, totalSelected }: ElementsListProps) {
    return (
        <>
            <InfoHeader className="justify-between">{totalSelected} selected</InfoHeader>
            <ul className="flex flex-col gap-y-3 grow min-h-0 overflow-y-auto">
                {selectedElements.map((selectedElement) => (
                    <ElementDetails key={selectedElement.id} element={selectedElement} expand={false} />
                ))}
            </ul>
        </>
    );
