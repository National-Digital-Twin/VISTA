...
export default function ElementDetails({ element, expand }: Readonly<ElementDefaultsProps>) {
    ...
    if (isError) {
        return <p className={styles.errorMessage}>An error has occured while fetching information for {element.uri}</p>;
    }
    ...
    if (isEmpty(element) || !details) {
        return <p className={styles.errorMessage}>Unable to retrieve details for unknown element or details not found</p>;
    }

    if (!expand) {
        return (
            <li className={styles.elementDetails}>
                <button aria-label={details.title} onClick={toggleDropdown} className={styles.elementDetailsButton}>
                    <Details expand={false} details={details} />
                </button>
                {showDropdown && (
                    <div className={styles.elementDetailsDropdown}>
                        <div className={styles.streetViewContainer}>
                            <span className={styles.streetViewLabel}>Visit Asset in Street View:</span>
