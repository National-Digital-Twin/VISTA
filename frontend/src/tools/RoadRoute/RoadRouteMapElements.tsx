...
export default function RoadRouteMapElements() {
  ...
  const handleEndPositionDragEnd = useCallback(
    ({ lngLat }: MarkerDragEvent) => setEndPosition(lngLat as LngLat),
    [setEndPosition],
  );
    ...
    <>
      {startPosition && (
        <Marker
          ...
          draggable
          onDragEnd={handleStartPositionDragEnd}
        />
      )}
      {endPosition && (
        <Marker
          ...
          draggable
          onDragEnd={handleEndPositionDragEnd}
        />
      )}
      {data && startPosition && endPosition && (
        <Source
          id="road-route"
          type="geojson"
          data={data.roadRoute.routeGeojson}
        >
          <Layer
            id="road-route"
            type="line"
