...
export class Table<T> {
    ...
    formatNodes(element: T) {
        return this.columns.map((column) => (column.display ? column.display(element) : column.format(element)));
    }
...
ASSET_TABLE.addColumn({
    name: 'dependent',
    title: 'Dependent',
    format: (asset) => `Count: ${asset.dependent.count}, Criticality Sum: ${asset.dependent.criticalitySum}`,
});
ASSET_TABLE.addColumn({
