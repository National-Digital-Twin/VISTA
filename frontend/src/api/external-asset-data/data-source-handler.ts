...
import { AssetClassFilter, AssetSpecification } from '@/hooks/queries/dataset-utils';
...
export abstract class DataSourceHandler {
        ...
        for (let attempt = 0; attempt <= retries; attempt++) {
            try {
                ...
                if (attempt < retries) {
                    ...
                    console.warn(`Retrying ${url}, attempt ${attempt + 1} after ${delay}ms`);
                    await this.sleep(delay);
                } else {
                    throw new Error(`Failed to fetch ${url} after ${retries + 1} attempts: ${message}`);
                }
            }
        }
    }
     ...
     */
    protected isMatchForAssetSpecificationFilters(assetSpecification: AssetSpecification, feature: Feature): boolean {
        if (assetSpecification.filters) {
            const filtersToMatch = assetSpecification.filters.length;
            const matchedFilters = assetSpecification.filters.filter((f) => this.isMatchForAssetSpecificationFilter(f, feature));
            return filtersToMatch === matchedFilters.length;
        }
        return true;
    }

    private isMatchForAssetSpecificationFilter(filter: AssetClassFilter, feature: Feature) {
        if (Array.isArray(filter.filterValue)) {
            for (const filterValue of filter.filterValue) {
                if (feature.properties && feature.properties[filter.filterName] === filterValue) {
                    return true;
                }
            }
        } else {
            return feature.properties && feature.properties[filter.filterName] === filter.filterValue;
        }
    }

    abstract buildUrlsForDataSource(assetSpecification: AssetSpecification): string[];

    abstract fetchDataForAssetSpecification(assetSpecification: AssetSpecification, url: string): Promise<Feature[]>;
}
