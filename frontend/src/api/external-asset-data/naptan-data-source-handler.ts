...
export class NaptanDataSourceHandler extends DataSourceHandler {
    ...
    public buildUrlsForDataSource(_assetSpecification: AssetSpecification): string[] {
        return [`/transparent-proxy/naptan/v1/access-nodes?dataFormat=csv&atcoAreaCodes=${this.locator}`];
    }
    ...
    public async fetchDataForAssetSpecification(assetSpecification: AssetSpecification, url: string): Promise<Feature[]> {
        ...
        const featureForAssetType = features.filter((f) => this.isMatchForAssetSpecificationFilters(assetSpecification, f));
        return featureForAssetType.reduce<Feature[]>((acc, f) => this.mergeAssetsWithSameNameAtSameLocation(acc, f, seenLocations), []);
    }
    ...
    private createGeoJsonPointFeature(stop: NaptanStop): Feature {
        const { Longitude: longitude, Latitude: latitude, ATCOCode: atcoCode, CommonName: name, ...properties } = stop;
        ...
        return {
            ...
            geometry: {
                type: 'Point',
                coordinates: [this.convertToNumberIfString(longitude), this.convertToNumberIfString(latitude)],
            },
            properties: {
                ...properties,
                name,
