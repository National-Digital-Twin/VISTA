...
export class CQCDataSourceHandler extends DataSourceHandler {
  public buildUrlsForDataSource(
    ...
    return [
      `/transparent-proxy/cqc/public/v1/locations?localAuthority=${this.locator}&careHome=Y&perPage=1000`,
    ];
  }
  ...
  public async fetchDataForAssetSpecification(
    ...
    while (currentUrl) {
      const response: LocationSearchResponse =
        await this.fetchFromUrl(currentUrl);
      allLocations.push(...response.locations);

      currentUrl = response.nextPageUri
        ? `/transparent-proxy/cqc${response.nextPageUri}`
        : null;
    }

    const locationDetails = await Promise.all(
      allLocations.map((location) =>
        this.fetchLocationDetails(location.locationId),
      ),
    );
  ...
  private async fetchLocationDetails(
    locationId: string,
