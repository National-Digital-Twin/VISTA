 ...
 */
export const fetchLiveAssets = async (assetSpecification: AssetSpecification): Promise<FeatureCollection> => {
    ...
    const allFeatures = await Promise.all(urls.map((url) => handler.fetchDataForAssetSpecification(assetSpecification, url)));
