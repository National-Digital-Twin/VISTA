...
export class OsNgdDataSourceHandler extends DataSourceHandler {
    ...
    protected buildQueryString(allFilters: Filters, filters: [string, string][] = []): string {
        ...
        let filter: string = filters.map(([key, val]) => `filter=${encodeURIComponent(key)}='${encodeURIComponent(val)}'`).join('&');

        if (allFilters.cqlFilter) {
            if (filters.length > 0) {
                console.warn('Do not specify other filters when using cqlFilter', allFilters);
            }
    ...
    protected transformUrl(url: string): string {
        const url_replaced_stem = url.replace('https://api.os.uk', '/transparent-proxy/os-ngd');
    ...
    public async fetchDataForAssetSpecification(assetSpecification: AssetSpecification, url?: string): Promise<Feature[]> {
        ...
        while (url !== undefined) {
            ...
            const ngdFeatureCollection: NGDFeatureCollection = await this.limit(() => currentUrl && this.fetchFromUrlWithRetry(currentUrl));
            const features = this.mapNGDToGeoJSON(ngdFeatureCollection, assetSpecification);
            allFeatures.push(...features);
            if (paginate) {
                url = this.getNextUrl(ngdFeatureCollection.links);
            } else {
                const onLastPage = assetSpecification.expectedCount - this.getOffset(currentUrl) < 100;
     ...
     */
    public mapNGDToGeoJSON(ngdData: NGDFeatureCollection, assetSpecification: AssetSpecification): Feature[] {
        return ngdData.features
            .filter((f) => this.isMatchForAssetSpecificationFilters(assetSpecification, f))
            .map((f) => {
                return {
                    ...f,
                    properties: {
