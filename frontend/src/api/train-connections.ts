...
export const fetchTrainDepartures = async (station: string) => {
  ...
  if (!response.ok) {
    throw new Error(
      "An error occurred while retrieving train departure data for the specified station.",
    );
  }
...
export const fetchTrainArrivals = async (station: string) => {
  ...
  if (!response.ok) {
    throw new Error(
      "An error occurred while retrieving train arrival data for the specified station.",
    );
  }
