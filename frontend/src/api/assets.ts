...
export const fetchDependents = async (assetUri: string) => {
    const queryParams = new URLSearchParams({ assetUri }).toString();
    const response = await fetch(createParalogEndpoint(`asset/dependents?${queryParams}`), fetchOptions);
    if (!response.ok) {
...
export const fetchProviders = async (assetUri: string) => {
    const queryParams = new URLSearchParams({ assetUri }).toString();
    const response = await fetch(createParalogEndpoint(`asset/providers?${queryParams}`), fetchOptions);
    if (!response.ok) {
...
export const fetchResidents = async (assetUri: string) => {
    const queryParams = new URLSearchParams({ assetUri }).toString();
    const response = await fetch(createParalogEndpoint(`asset/residents?${queryParams}`), fetchOptions);
    if (!response.ok) {
