...
import { AssetClassification, AssetClassificationsByType } from './AssetClassification';
...
export default class Asset {
    ...
    get criticalityColor() {
        if (this.assetClassificationPriority && this.#classificationColorScale) {
            return getHexColor(this.#classificationColorScale, this.assetClassificationPriority);
        } else {
    ...
    createLinearAsset<T extends ElementLike>(selectedElements: T[]): Feature | null {
        const geometry: Geometry = this.getLinearGeometry();
        if (this.geometry.type === 'MultiLineString' || this.geometry.type === 'LineString') {
