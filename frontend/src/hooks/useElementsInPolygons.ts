...
import { circle as turfCircle, booleanPointInPolygon, booleanIntersects } from '@turf/turf';
...
export default function useElementsInPolygons() {
    const findElementsInPolygons = ({ target, polygons, assets, dependencies }) => {
        ...
        const elements = [...pointsInPolygon, ...connectedAssets, ...LAIntersectingPolygon, ...connectedDependencies].map((element) => {
