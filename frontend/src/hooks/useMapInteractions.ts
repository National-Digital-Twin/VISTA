...
import { FLOOD_AREA_POLYGON_ID, FLOOD_AREA_POLYGON_OUTLINE_ID, LINEAR_ASSET_LAYER, pointAssetCxnLayer } from '@/components/Map/layers';
...
export default function useMapInteractions({
    ...
    const onFloodZoneClick = ({ clickedFloodZones, isMultiSelect }: { clickedFloodZones: Feature[]; isMultiSelect: boolean }) => {
...
const getAllSelectedPolygons = (selected: Feature[]) => {
    return selected.reduce<Feature[]>((acc, current) => {
        if (!acc.some((feature) => feature.properties?.id === current.properties?.id)) {
            acc.push(current);
        }
        return acc;
    }, []);
