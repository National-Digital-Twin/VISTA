...
import type { Feature, GeoJsonProperties, LineString, MultiLineString, MultiPoint, MultiPolygon, Point, Polygon } from 'geojson';
 ...
 */
function isPolygonFeature(feature: Feature): feature is Feature<Polygon, GeoJsonProperties> {
 ...
 */
function isMultiPolygonFeature(feature: Feature): feature is Feature<MultiPolygon, GeoJsonProperties> {
 ...
 */
function isMultiPointFeature(feature: Feature): feature is Feature<MultiPoint, GeoJsonProperties> {
 ...
 */
function isMultiLineStringFeature(feature: Feature): feature is Feature<MultiLineString, GeoJsonProperties> {
...
function getCoordinatesforPointAsset(feature: Feature): number[] {
    if (isPolygonFeature(feature) || isMultiPolygonFeature(feature)) {
        ...
        return coordinates[0].map((_, i) => coordinates.reduce((sum, p) => sum + p[i], 0) / coordinates.length);
    } else {
        return [0, 0];
    }
}
 ...
 */
function mapPointAsset(feature: Feature, assetSpecification: AssetSpecification, staticDataForAssetClass: any[]): Asset {
 ...
 * @returns an instance of Asset
 */
function mapLinearAsset(feature: Feature, assetSpecification: AssetSpecification): Asset {
    const type = assetSpecification.type;
    const description = assetSpecification.type.includes('#') ? assetSpecification.type.split('#')[1] : '';
    const asset = new Asset({
...
async function fetchDataForAssetClass(assetUris: string[]): Promise<any[]> {
    return await Promise.all(assetUris.map(async (uri: string): Promise<any> => await fetchAssetInfo(getAssetUri(uri))));
}

export async function fetchAssetsForAssetSpecification(assetSpecification: AssetSpecification) {
