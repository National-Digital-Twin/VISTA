...
export default function FloodMonitoringStations({ query, showStations = false }: FloodMonitoringStationsProps) {
    const [selectedStation, setSelectedStation] = useState<Feature | undefined>(undefined);
    ...
    const handleOnStationClick = (feature: Feature) => setSelectedStation(feature);
    const handleOnClosePopup = () => setSelectedStation(undefined);

    return (
        <Source id="flood-monitoring-stations" type={GEOJSON} data={{ type: FEATURE_COLLECTION, features }}>
...
function StationPopup({ selectedStation, onClose }: StationPopupProps) {
    ...
    return (
        ...
        >
            ...
            <a
                href={`https://environment.data.gov.uk/flood-monitoring/id/stations/${getId(selectedStation.properties?.id ?? '')}.html`}
                target="_blank"
                rel="noreferrer"
