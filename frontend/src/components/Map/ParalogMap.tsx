import { useContext, useEffect, useMemo, useState, lazy, memo, Suspense } from 'react';
import type { Feature } from 'geojson';
import Map, { Layer, LayerProps, Source, ScaleControl, useMap, AttributionControl, NavigationControl } from 'react-map-gl/maplibre';
...
const generateSources = (source: ToolSourceType) => (
    <Source key={source.id} id={source.id} type={GEOJSON} data={{ type: FEATURE_COLLECTION, features: source.features }} generateId>
...
function BuiltinSources() {
    ...
    const sources = useMemo(() => {
        ...
        allSources.push({
            id: 'flood-areas',
            features: [...floodAreas.flat(), ...liveFloodAreas],
...
export default function ParalogMap() {
    ...
    const [mapStyleKey, setMapStyleKey] = useLocalStorage('mapStyle', mapStyles[0].key);
    ...
    return (
        ...
        >
            <MapStyleContextProvider mapStyleKey={mapStyleKey} setMapStyleKey={setMapStyleKey}>
                <ShowPointerCoordsContextProvider showPointerCoords={showPointerCoords} setShowPointerCoords={setShowPointerCoords}>
                                ...
                                {showBuildingLayer && (
                                    <Layer
                                        ...
                                        paint={{
                                            ...
                                            'fill-extrusion-opacity': ['interpolate', ['linear'], ['zoom'], 15, 0, 16, 0.9],
                                        }}
                                    />
                                )}
