    ...
    zoomableData.forEach((item) => {
      if (item.time && typeof item.time === "string") {
        ...
        if (
          date &&
          !ticks.some(
            (existingItem) => existingItem.time.split("T")[0] === date,
          )
        ) {
          ticks.push(item);
        }
  ...
  return {
    ...
    xAxisProps: enabled
      ? {
          ...
          tickFormatter: (milliseconds: number) =>
            dayjs(milliseconds).format("D MMM YY, HH:mm"),
          tickCount: 10,
        }
      : {
          domain,
          tickFormatter: (milliseconds: number) =>
            dayjs(milliseconds).format("D MMM YY"),
          ticks: legacyTicks,
        },
    yAxisProps: enabled
      ? {
