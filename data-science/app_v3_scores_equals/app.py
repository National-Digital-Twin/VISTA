...
def compute_exposure_score(assets_df, flood_files, heat_files=None, landslide_files=None):
    ...
    for index, asset in assets_gdf.iterrows():
        ...
        intersects_flood = bool(flood_geom and not flood_geom.is_empty and asset_geom.intersects(flood_geom))
        intersects_heat = bool(heat_geom and not heat_geom.is_empty and asset_geom.intersects(heat_geom))
        intersects_landslide = bool(landslide_geom and not landslide_geom.is_empty and asset_geom.intersects(landslide_geom))

        # Count the number of intersections (True values are treated as 1, False as 0)
        intersection_count = sum([intersects_flood, intersects_heat, intersects_landslide])

        if intersection_count >= 2:
            score = 3
        elif intersects_flood: # This implies intersection_count is 1 and it's the flood layer
            score = 2
        elif flood_geom_proj: # This implies no intersection with flood
            # Check for proximity to flood only if there are no intersections at all
